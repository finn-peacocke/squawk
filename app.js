...
async function runParseAgent() {
  ...
  // Get all sources that haven't been enriched yet (no AI-assigned date)
  // Mark enriched sources with s.enriched = true to prevent re-runs
  var toEnrich = sources.filter(function(s) { return !s.enriched; });

  if (!toEnrich.length) {
    resultEl.textContent = 'All sources already enriched. Reload your sheet to re-enrich.';
    resultEl.style.display = 'block';
    btn.disabled = false;
    btn.textContent = 'Run agent';
    return;
  ...
  var systemPrompt = 'You are a senior deal flow analyst for GForce Ventures — a GLOBAL pre-seed/seed investor.'
    + ' GForce focus: Energy Technologies, DeepTech, Materials AI, SpaceTech, Robotics, Energy-efficient compute.'
    + ' Today is ' + now.toDateString() + '.'
    + '\n\nFor each programme, find the single MOST IMPORTANT upcoming date: application open date, application deadline, cohort start, demo day, or event date.'
    + ' Use your deep training knowledge of each programme. Be specific — give a real date in YYYY-MM-DD format.'
    + ' For annual recurring programmes, project forward to the next occurrence after today.'
    + ' For events, name 2-3 typical attendee types or past notable speakers.'
    + '\n\nReturn ONLY a valid JSON array, no prose, no markdown fences.'
    + '\n[{"name":"exact name","next_date":"YYYY-MM-DD or null","date_label":"Applications open|Deadline|Event date|Cohort start|Demo day","confidence":"high|medium|estimate","attendees":"null or attendee description","insight":"one sentence deal flow value for GForce"}]';

  // Process in batches of 20 across ALL tabs
  var BATCH_SIZE = 20;
  var allSummary = [];
  var totalUpdated = 0;
  var batchNum = 0;

  btn.textContent = 'Enriching 0/' + toEnrich.length + '...';

  while (batchNum * BATCH_SIZE < toEnrich.length) {
    var batch = toEnrich.slice(batchNum * BATCH_SIZE, (batchNum + 1) * BATCH_SIZE);
    batchNum++;
    btn.textContent = 'Enriching ' + Math.min(batchNum * BATCH_SIZE, toEnrich.length) + '/' + toEnrich.length + '...';

    var sample = batch.map(function(s) {
      return s.name + ' | type:' + s.type + ' | tab:' + (s.tab || '') + ' | url:' + (s.url || 'none') + ' | existing date:' + (s.rawDate || 'none');
    }).join('\n');

    var userPrompt = 'Find the next key date for each of these ' + batch.length + ' deal flow sources. Today: ' + now.toDateString() + '\n\n' + sample;
    ...
    try {
      ...
      enrichments.forEach(function(e) {
        ...
        if (!source) return;
        source.enriched = true; // mark as done — won't re-enrich
        if (e.next_date) {
        ...
        if (e.attendees && e.attendees !== 'null') source.attendees = e.attendees;
        var conf = e.confidence === 'high' ? '✓' : e.confidence === 'medium' ? '~' : '?';
        allSummary.push(conf + ' ' + e.name + ': ' + (e.next_date || 'unknown') + (e.date_label ? ' (' + e.date_label + ')' : ''));
      });

      // Re-render after each batch so calendar updates progressively
      renderAll();
      showToast('Enriched batch ' + batchNum + ' — ' + totalUpdated + ' dates found so far');

      // Small delay between batches to avoid rate limits
      if (batchNum * BATCH_SIZE < toEnrich.length) {
        await new Promise(function(r) { setTimeout(r, 1000); });
      }

    } catch(err) {
      allSummary.push('ERROR in batch ' + batchNum + ': ' + err.message);
    }
  }

  resultEl.textContent = 'Enriched ' + totalUpdated + ' of ' + toEnrich.length + ' sources with dates.\n\n' + allSummary.join('\n');
  resultEl.style.display = 'block';
  showToast('Enrichment complete — ' + totalUpdated + ' dates added');
