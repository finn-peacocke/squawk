...
function parseEventsTab(rows, tabName) {
  return rows.slice(1)
    ...
    .map(function(row) {
      ...
      return { name: name, url: '', type: 'event', rawDate: rawDate, rawEndDate: rawEndDate, parsedDate: parsedDate, notes: notes, urgency: urgency, tab: tabName };
    });
}
...
function runParseAgent() {
  ...
  var sample = sources.slice(0, 25).map(function(s) {
    return '- ' + s.name + ' | type: ' + s.type + ' | tab: ' + (s.tab || 'unknown') + ' | url: ' + (s.url || 'none') + ' | date: ' + (s.rawDate || 'none') + ' | notes: ' + (s.notes || 'none');
  }).join('\n');

  callClaude(
    'You are a deal flow analyst for an early-stage investor. Today is ' + now.toDateString() + '.\n' +
    'Your job is to enrich deal flow sources with:\n' +
    '1. DATES — next cohort open/close dates, application deadlines, announcement dates. Use your knowledge of these programmes. Give exact dates where known, or realistic estimates based on historical patterns.\n' +
    '2. KEY ATTENDEES / SPEAKERS — for events, name typical calibre of attendees, notable past speakers, or target audience.\n' +
    '3. URGENCY — urgent (within 7 days), soon (within 30 days), monitor (1–3 months), low (3+ months).\n' +
    '4. INSIGHT — one sharp sentence on why this matters for early-stage deal flow.\n\n' +
    'Be specific and direct. Use your knowledge — do not be vague.',
...
function parseMarkdownSuggestions(text) {
  var suggestions = [];
  // Split on ## or numbered headers like "1." / "**1.**"
...
\d+\.\s)/);
  blocks.forEach(function(block) {
    ...
    var nameLine = block.split('\n')[0].replace(/^#+\s*/, '').replace(/\*\*/g, '').replace(/^\d+\.\s*/, '').trim();
...
-]+)/);
    ...
    var why = whyMatch ? whyMatch[1].replace(/\*\*/g, '').trim() : block.split('\n').slice(-1)[0].replace(/\*\*/g, '').trim();
