...
async function addToCalendar(id) {
  ...
  // End date for all-day must be the day AFTER (Google Calendar convention)
  var endDate = s.rawEndDate ? (parseDate(s.rawEndDate) || start) : start;
  var endNext = new Date(endDate);
  endNext.setDate(endNext.getDate() + 1);
  var endStr = toDateStr(endNext);
...
// ── Enrichment card state ─────────────────────────────────────────────────
var ENRICH_KEY  = 'squawk_enrichments';
var enrichPage_ = 0;
var ENRICH_PER_PAGE = 20;
var enrichedResults = []; // full list of enrichment objects for display

function loadEnrichments() {
  try { return JSON.parse(localStorage.getItem(ENRICH_KEY)) || {}; } catch { return {}; }
}
function saveEnrichments(obj) {
  localStorage.setItem(ENRICH_KEY, JSON.stringify(obj));
}
function clearEnrichments() {
  localStorage.removeItem(ENRICH_KEY);
  enrichedResults = [];
  sources.forEach(function(s) { s.enriched = false; s.parsedDate = null; s.rawDate = ''; s.urgency = 'unknown'; });
  document.getElementById('enrichCards').innerHTML = '';
  document.getElementById('enrichPagination').style.display = 'none';
  renderAll();
  showToast('Enrichments cleared — reload sheet to re-enrich.');
}

// Restore enrichments from localStorage on sheet load
function restoreEnrichments() {
  var stored = loadEnrichments();
  var keys = Object.keys(stored);
  if (!keys.length) return;
  keys.forEach(function(name) {
    var e = stored[name];
    var source = sources.find(function(s) { return s.name.trim().toLowerCase() === name.toLowerCase(); });
    if (!source) return;
    source.enriched = true;
    if (e.next_date) {
      var parsed = parseDate(e.next_date);
      if (parsed) { source.parsedDate = parsed; source.rawDate = (e.date_label || '') + ': ' + e.next_date; source.urgency = getUrgency(parsed); }
    }
    if (e.insight) source.notes = e.insight + (e.attendees && e.attendees !== 'null' ? ' · ' + e.attendees : '');
  });
  enrichedResults = keys.map(function(n) { return stored[n]; });
  renderAll();
  if (enrichedResults.length) renderEnrichCards();
}
...
async function runParseAgent() {
  ...
  var progressEl  = document.getElementById('enrichProgress');
  var progressBar = document.getElementById('enrichProgressBar');
  var progressLbl = document.getElementById('enrichProgressLabel');
  var progressCnt = document.getElementById('enrichProgressCount');

  // Guard: if any element is missing, the agent tab isn't visible — switch to it first
  if (!btn || !resultEl || !progressEl) {
    switchTab('agent');
    setTimeout(runParseAgent, 100);
    return;
  }
  ...
  var toEnrich = sources.filter(function(s) { return !s.enriched; });
  if (!toEnrich.length) {
    resultEl.textContent = 'All sources already enriched. Click "Reset all enrichments" to start fresh.';
    resultEl.style.display = 'block';
    progressEl.style.display = 'none';
    btn.disabled = false; btn.textContent = 'Run enrichment';
    return;
  ...
  var systemPrompt = 'You are a senior deal flow analyst for GForce Ventures, a GLOBAL pre-seed/seed investor focused on Energy Technologies, DeepTech, Materials AI, SpaceTech, Robotics, and Energy-efficient compute.'
    + ' Today is ' + now.toDateString() + '.'
    + '\n\nYour task: find the next key date for each programme. NEVER return null for next_date. You must always provide a best estimate.'
    + '\n\nREASONING RULES — work through these in order for each source:'
    + '\n1. If you know the exact date from training data, use it.'
    + '\n2. If you know the programme runs annually and you know the typical month, project to the next occurrence after today. e.g. if it usually opens in September, return the next September 1st after today.'
    + '\n3. If you only know the rough season (spring/autumn), use the midpoint: spring=April 1, summer=July 1, autumn=October 1, winter=January 1.'
    + '\n4. If you know nothing about the programme specifically, reason from its TYPE and COUNTRY:'
    + '\n   - French accelerators/incubators: most run Sept-Oct or Jan-Feb intake cycles. Default to next Sept 1.'
    + '\n   - French grants (BPI etc): rolling or quarterly. Default to next quarter start.'
    + '\n   - UK accelerators: typically Jan or Sept cohorts. Default to next Sept 1.'
    + '\n   - EU competitions: typically spring deadlines (April-May). Default to next April 1.'
    + '\n   - Global deeptech accelerators: typically two cohorts/year, Jan and Sept. Default to next Sept 1.'
    + '\n   - VC studios/residencies: rolling or quarterly. Default to next quarter start.'
    + '\n   - Conferences/events: look at the URL or name for clues. Annual events repeat same time each year.'
    + '\n5. NEVER return null. If genuinely unknown, use the type/country rule above and set confidence to "estimate".'
    + '\n\nFor events (type=event), also identify 2-3 typical attendee profiles or notable past speakers from your knowledge.'
    + '\nFor each source write a one-sentence insight on why it matters for GForce deal flow specifically.'
    + '\n\nReturn ONLY a valid JSON array, no prose, no markdown.'
    + '\n[{"name":"exact name as given","next_date":"YYYY-MM-DD","date_label":"Applications open|Deadline|Event date|Cohort start|Demo day|Grant window","confidence":"high|medium|estimate","attendees":"description or null","insight":"one sentence"}]';

  var BATCH = 20;
  var totalBatches = Math.ceil(toEnrich.length / BATCH);
  var stored = loadEnrichments();
  var totalUpdated = 0;

  for (var b = 0; b < totalBatches; b++) {
    var batch = toEnrich.slice(b * BATCH, (b + 1) * BATCH);
    var pct   = Math.round((b / totalBatches) * 100);
    progressBar.style.width = pct + '%';
    progressLbl.textContent = 'Enriching batch ' + (b + 1) + ' of ' + totalBatches + '...';
    progressCnt.textContent = totalUpdated + ' dates found';

    var sample = batch.map(function(s) {
      // Include as many context clues as possible to help the agent reason
      var parts = [s.name, 'type:' + s.type, 'country/tab:' + (s.tab || 'unknown'), 'url:' + (s.url || 'none')];
      if (s.notes) parts.push('notes:' + s.notes);
      return parts.join(' | ');
    }).join('\n');

    try {
      progressLbl.textContent = 'Calling API for batch ' + (b + 1) + ' of ' + totalBatches + '...';
      resultEl.textContent = 'Batch ' + (b+1) + '/' + totalBatches + ' — waiting for API response...';
      resultEl.style.display = 'block';
      ...
      var res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          system: systemPrompt,
          messages: [
            { role: 'user', content: 'For each of these ' + batch.length + ' deal flow sources, reason through the date using the rules in your instructions. Today is ' + now.toDateString() + '. Remember: NEVER return null for next_date — always give a best estimate.\n\n' + sample },
      ...
      var data = await res.json();
      if (data.error) throw new Error('API error ' + data.error.status + ': ' + data.error.message);
      var text = '[' + (data.content && data.content[0] ? data.content[0].text : '');
      var jsonMatch = text.match(/\[[\s\S]*\]/);
      if (!jsonMatch) {
        resultEl.textContent = 'Batch ' + (b+1) + ': no JSON in response. Raw: ' + text.substring(0, 200);
        continue;
      }
      var enrichments;
      try { enrichments = JSON.parse(jsonMatch[0]); }
      catch(parseErr) {
        resultEl.textContent = 'Batch ' + (b+1) + ': JSON parse error — ' + parseErr.message;
        continue;
      }
      resultEl.textContent = 'Batch ' + (b+1) + ' parsed — ' + enrichments.length + ' results received.';
      ...
      enrichments.forEach(function(e) {
        ...
        if (e.next_date) {
          ...
          if (parsed) {
            ...
            source.rawDate    = (e.date_label || '') + ': ' + e.next_date;
            source.urgency    = getUrgency(parsed);
            totalUpdated++;
          }
        }
        if (e.insight) source.notes = e.insight + (e.attendees && e.attendees !== 'null' ? ' · ' + e.attendees : '');
        stored[e.name.trim().toLowerCase()] = e;
        enrichedResults.push(e);
      });

      saveEnrichments(stored);
      renderAll();
      renderEnrichCards();
      ...
      var errMsg = 'Batch ' + (b+1) + ' error: ' + err.message;
      console.error(errMsg);
      resultEl.textContent = errMsg;
      resultEl.style.display = 'block';
      // If it's an auth error, stop entirely
      if (err.message.includes('401') || err.message.includes('api-key') || err.message.includes('aborted')) {
        showToast('Enrichment stopped: ' + err.message);
        break;
      }
    }
  }
  ...
  progressLbl.textContent = 'Enrichment complete';
  progressCnt.textContent = totalUpdated + ' dates found';
  setTimeout(function() { progressEl.style.display = 'none'; }, 2000);
  showToast('Enrichment complete — ' + totalUpdated + ' dates added across all tabs');
  btn.disabled = false;
  btn.textContent = 'Run enrichment';
}

function enrichPage(dir) {
  var maxPage = Math.ceil(enrichedResults.length / ENRICH_PER_PAGE) - 1;
  enrichPage_ = Math.max(0, Math.min(maxPage, enrichPage_ + dir));
  renderEnrichCards();
}

function renderEnrichCards() {
  var container  = document.getElementById('enrichCards');
  var pagination = document.getElementById('enrichPagination');
  var pageLabel  = document.getElementById('enrichPageLabel');
  var stats      = document.getElementById('enrichStats');
  var prevBtn    = document.getElementById('enrichPrevBtn');
  var nextBtn    = document.getElementById('enrichNextBtn');

  if (!enrichedResults.length) { container.innerHTML = ''; pagination.style.display = 'none'; return; }

  var maxPage   = Math.ceil(enrichedResults.length / ENRICH_PER_PAGE) - 1;
  enrichPage_   = Math.max(0, Math.min(maxPage, enrichPage_));
  var pageItems = enrichedResults.slice(enrichPage_ * ENRICH_PER_PAGE, (enrichPage_ + 1) * ENRICH_PER_PAGE);
  var withDates = enrichedResults.filter(function(e) { return e.next_date; }).length;

  pagination.style.display = 'block';
  pageLabel.textContent    = 'Page ' + (enrichPage_ + 1) + ' of ' + (maxPage + 1);
  stats.textContent        = withDates + ' of ' + enrichedResults.length + ' sources have dates';
  prevBtn.disabled         = enrichPage_ === 0;
  nextBtn.disabled         = enrichPage_ === maxPage;

  container.innerHTML = '';
  pageItems.forEach(function(e) {
    var confClass = e.confidence === 'high' ? 'confidence-high' : e.confidence === 'medium' ? 'confidence-medium' : 'confidence-estimate';
    var confLabel = e.confidence === 'high' ? 'high' : e.confidence === 'medium' ? 'medium' : 'estimate';
    var confBadge = '<span class="enrich-card-confidence conf-' + confLabel + '">' + confLabel + '</span>';
    var cardId    = 'ecard-' + encodeURIComponent(e.name);
    var stored    = loadEnrichments();
    var reviewed  = stored[e.name.trim().toLowerCase()] && stored[e.name.trim().toLowerCase()].reviewed;

    var card = document.createElement('div');
    card.className = 'enrich-card ' + confClass + (reviewed ? ' reviewed' : '');
    card.id = cardId;
    card.innerHTML =
      '<div class="enrich-card-header">' +
        '<div style="flex:1">' +
          '<div class="enrich-card-name">' + esc(e.name) + confBadge + '</div>' +
          '<div style="font-size:11px;color:#bbb;">' + esc(e.tab || '') + (e.type ? ' · ' + esc(e.type) : '') + (e.url ? ' · <a href="' + esc(e.url) + '" target="_blank" style="color:#3b5bdb;text-decoration:none;">' + shortUrl(e.url) + '</a>' : '') + '</div>' +
        '</div>' +
      '</div>' +
      (e.next_date ?
        '<div class="enrich-card-date">📅 <strong>' + esc(e.next_date) + '</strong><span class="date-label">' + esc(e.date_label || '') + '</span></div>' :
        '<div class="enrich-card-date" style="color:#aaa;">No date found</div>'
      ) +
      (e.insight    ? '<div class="enrich-card-insight">'   + esc(e.insight)    + '</div>' : '') +
      (e.attendees && e.attendees !== 'null' ? '<div class="enrich-card-attendees">👥 ' + esc(e.attendees) + '</div>' : '') +
      '<div class="enrich-card-actions">' +
        '<button class="btn btn-sm btn-accept" onclick="reviewEnrichment(\'' + encodeURIComponent(e.name) + '\', true)">✓ Looks good</button>' +
        '<button class="btn btn-sm" onclick="toggleEnrichEdit(\'' + encodeURIComponent(e.name) + '\')" style="color:#555;">✏ Edit</button>' +
      '</div>' +
      '<div class="enrich-edit-row" id="edit-' + encodeURIComponent(e.name) + '">' +
        '<input type="text" placeholder="YYYY-MM-DD" value="' + esc(e.next_date || '') + '" id="editdate-' + encodeURIComponent(e.name) + '" />' +
        '<input type="text" placeholder="Label (e.g. Deadline)" value="' + esc(e.date_label || '') + '" id="editlabel-' + encodeURIComponent(e.name) + '" style="flex:0.6;" />' +
        '<button class="btn btn-sm btn-primary" onclick="saveEnrichEdit(\'' + encodeURIComponent(e.name) + '\')">Save</button>' +
        '<button class="btn btn-sm" onclick="toggleEnrichEdit(\'' + encodeURIComponent(e.name) + '\')">Cancel</button>' +
      '</div>';
    container.appendChild(card);
  });
}

function reviewEnrichment(encodedName, accepted) {
  var name   = decodeURIComponent(encodedName);
  var stored = loadEnrichments();
  var key    = name.trim().toLowerCase();
  if (stored[key]) { stored[key].reviewed = true; saveEnrichments(stored); }
  var card = document.getElementById('ecard-' + encodedName);
  if (card) card.classList.add('reviewed');
  showToast('Marked as reviewed');
}

function toggleEnrichEdit(encodedName) {
  var row  = document.getElementById('edit-' + encodedName);
  var card = document.getElementById('ecard-' + encodedName);
  if (!row) return;
...
function saveEnrichEdit(encodedName) {
  var name      = decodeURIComponent(encodedName);
  var newDate   = document.getElementById('editdate-'  + encodedName).value.trim();
  var newLabel  = document.getElementById('editlabel-' + encodedName).value.trim();
  var stored    = loadEnrichments();
  var key       = name.trim().toLowerCase();

  if (stored[key]) {
    stored[key].next_date   = newDate;
    stored[key].date_label  = newLabel;
    stored[key].reviewed    = true;
    stored[key].confidence  = 'high';
    saveEnrichments(stored);
  }

  // Also update the live source
  var source = sources.find(function(s) { return s.name.trim().toLowerCase() === key; });
  if (source && newDate) {
    var parsed = parseDate(newDate);
    if (parsed) {
      source.parsedDate = parsed;
      source.rawDate    = (newLabel || '') + ': ' + newDate;
      source.urgency    = getUrgency(parsed);
    }
  }

  // Update enrichedResults display copy
  var er = enrichedResults.find(function(e) { return e.name.trim().toLowerCase() === key; });
  if (er) { er.next_date = newDate; er.date_label = newLabel; er.confidence = 'high'; }

  renderAll();
  renderEnrichCards();
  showToast('Date updated for ' + name);
}
