...
async function callClaude(system, user, btnId, resultId) {
  ...
  // Always read key fresh — from variable, input field, or localStorage
  const keyToUse = anthropicKey
    || (document.getElementById('anthropicKeyInput') ? document.getElementById('anthropicKeyInput').value.trim() : '')
    || (loadConfig().anthropicKey || '');

  if (!keyToUse) {
    result.textContent = 'Please enter your Anthropic API key — either in the setup screen or via the API Key button in the header.';
  ...
  // Keep the variable in sync for future calls
  anthropicKey = keyToUse;
